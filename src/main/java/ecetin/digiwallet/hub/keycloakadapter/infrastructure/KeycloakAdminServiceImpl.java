    ...
    public void createUser(KeycloakUser user) {
        try {
            ...
            if (user.getCredentials() != null && !user.getCredentials().isEmpty()) {
                List<CredentialRepresentation> credentials = user.getCredentials().stream()
            ...
            if (response.getStatus() == 201) {
                ...
                log.error("Failed to create user in Keycloak: {} - Status: {}", user.getUsername(), response.getStatus());
                response.close();
